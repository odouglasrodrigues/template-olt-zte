...
def CronModify(ip, user, password, port, hostname):
    ...
    time.sleep(.4)
    os.system(
        'sudo echo "27 */2 * * * zabbix python3 -u /usr/lib/zabbix/externalscripts/GetONUSignal-ZTE-C600.py {} {} {} {} {} &">>/etc/cron.d/TemplateOLT'
        .format(ip, user, password, port, hostname))
    time.sleep(.4)
    os.system(
        'sudo echo "*/6 * * * * zabbix python3 -u /usr/lib/zabbix/externalscripts/GetONUOnline-ZTE-C600.py {} {} {} {} {} &">>/etc/cron.d/TemplateOLT'
        .format(ip, user, password, port, hostname))
    time.sleep(.4)
...
def main(ip, community):
    ...
    for linha in return_snmpwalk:
        ...
        cmd_alias = 'snmpwalk -v 2c -c {} {}:65161 1.3.6.1.2.1.31.1.1.1.18.{}'.format(
            community, ip, index)
        alias_return = os.popen(cmd_alias).read()
        alias = ""
        if "STRING" in alias_return:
            alias = alias_return.split(':')[1].replace(' ', '').replace('\\', '').replace(
                '"', '').lstrip().rstrip()
        export["data"].append({
            "{#PONNAME}": pon,
            "{#PONALIAS}": alias,
